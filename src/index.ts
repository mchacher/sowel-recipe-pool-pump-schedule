...
export function createRecipe(): RecipeDefinition {
  return {
    ...
    createInstance(params, ctx) {
       ...
       *
       * Deliberately absent: any surplus test of the recipe's own (#22). While
       * the claim is granted, whether the surplus still covers it is the
       * arbiter's question, not this recipe's.
       */
      ...
      function manageHeaterClaim(): void {
        ...
        const want =
          ...
          arbiterGranted &&
          heatingNeeded() &&
          // #22 — no surplus test here. The recipe asks for capacity and holds
          // the claim until the arbiter takes it back; the arbiter alone owns
          // the exit. v1.6.3 watched the signed grid balance and released the
          // claim itself after ~3 min of import, to escape the heater's own
          // `minOnS` shield. That made the recipe a second, faster surplus
          // authority: the arbiter never revoked this load (27 grants, 0
          // revokes over a week on the reference installation), so its timeline
          // showed a mute "libéré" instead of "surplus retiré", its short-cycle
          // metric counted nothing, and `minOffS` never armed. The shield is
          // configuration, not code: set the heat pump's `minOnS` to 0 and the
          // arbiter revokes on its own `releaseHoldS`.
          ctx.state.get("override") !== true &&
          ctx.state.get("heaterOverride") !== true;
